...
-- Config of devices
--
dev=m:section(TypedSection,"mydevice","Devices")
dev.addremove=true
dev.anonymous=false

uid = dev:option(Value ,"uid",translate("UID"))
uid.optional = false;
uid.rmempty = false;

abp = dev:option(ListValue,"abp",translate("ABP"), "Authentication by personalization")
abp.optional = false;
abp.rmempty = false;
abp.default = true
abp.datatype = "bool"
abp:value(true, translate("True"))
abp:value(false, translate("False"))

appKey = dev:option(Value,"appKey",translate("AppKey"))
appKey.optional = false;
appKey.rmempty = false;

nwkskey = dev:option(Value,"nwkskey",translate("NwkSKey"))
nwkskey.optional = false;
nwkskey.rmempty = true;

appskey = dev:option(Value,"appskey",translate("AppSKey"))
appskey.optional = false;
appskey.rmempty = true;

devAddr = dev:option(Value,"devAddr",translate("devAddr"))
devAddr.optional = false;
devAddr.rmempty = false;

decoder = dev:option(Value,"decoder",translate("Decoder"), "Script to be executed after packet decryption and verification")
decoder.optional = false;
decoder.rmempty =false;
...
gateway:tab("ttnapi", translate("TTN API"))
gateway:tab("gcp", translate("GCP IoT"))
...
gateway:taboption("ttnapi", Value,"application",translate("TTN application"))
gateway:taboption("ttnapi", Value,"apiurl",translate("TTN API URL"))
gateway:taboption("ttnapi", Value,"apiport",translate("TTN API port"))
gateway:taboption("ttnapi", Value,"authkey",translate("TTN API authkey"))

gateway:taboption("gcp", Value,"gcp_project_id",translate("Project ID"))
local gcp_conf = gateway:taboption("gcp", Value,"gcp_cloud_region",translate("Cloud Region"))
gcp_conf:value("us-central1", "Central USA")
gcp_conf:value("europe-west1", "Western Europe")
gcp_conf:value("asia-east1", "Eastern Asia")
gateway:taboption("gcp", Value,"gcp_registry_id",translate("Registry ID"))
gateway:taboption("gcp", Value,"gcp_device_id",translate("Device ID"))
keyupload = gateway:taboption("gcp", FileUpload, "gcp_private_key_file", translate("Private key (RS256)"))
keyupload.root_directory = "/etc/lora/certs"
--
-- forward_crc_valid
--
